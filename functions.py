...
def setipclient(e,ip,*args):
    ...
    for item in classes.list_item2_selected:
        ...
        if path.is_dir():
            socket_send_command(ipclient=classes.ipclient,pre="folder",item=item,size=size)
        else:
            socket_send_command(ipclient=classes.ipclient,pre="file",item=item,size=size)
        ...
        with open(local_path, "wb") as f:
            ...
            for chunk in r.iter_content(chunk_size=8192):
                ...
                if chunk:
                    ...
                    index=classes.upload_infos[ipserver]
